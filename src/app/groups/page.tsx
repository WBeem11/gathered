...
export default function GroupsPage() {
  ...
  const [form, setForm] = useState({ name: "", description: "", type: "bible_study", meetingFrequency: "", location: "", isVirtual: false, link: "" });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setShowForm(false);
      setForm({ name: "", description: "", type: "bible_study", meetingFrequency: "", location: "", isVirtual: false, link: "" });
    }
    setSubmitting(false);
  }
    ...
    <div className="container mx-auto px-4 py-6 max-w-3xl">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-navy/60 max-w-md mx-auto">
          Life is better together. Find a group to grow in faith and friendship.
        </p>
      </div>
      ...
      <div className="flex justify-end mb-6">
        {session ? (
          ...
          <Link href="/sign-in">
            <Button variant="outline" className="border-navy/30 text-navy">Sign In to Add</Button>
          </Link>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <p className="font-playfair text-xl text-navy/50">No groups yet — be the first to add one!</p>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {groups.map((group) => {
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {group.link && <ExternalLink className="w-4 h-4 text-sage shrink-0 mt-1" />}
                    </div>
                    ...
                    {group.link && (
                      <div className="mt-3 pt-3 border-t border-cream">
                        <span className="text-xs font-medium text-sage flex items-center gap-1">
                          <ExternalLink className="w-3 h-3" /> Join this group →
                        </span>
                      </div>
                    )}
                  </div>
            ...
            return group.link ? (
              <a key={group.id} href={group.link} target="_blank" rel="noopener noreferrer">
                {card}
              </a>
            ) : (
              <div key={group.id}>{card}</div>
            );
          })}
        </div>
      )}
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-playfair text-navy">Add a Group</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.link} onChange={e => setForm({...form, link: e.target.value})} placeholder="https://facebook.com/groups/... or GroupMe, WhatsApp, etc." />
              <p className="text-xs text-muted-foreground">Paste a Facebook Group, GroupMe, WhatsApp, or any link people can click to join.</p>
            </div>
